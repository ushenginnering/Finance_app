...
// Get all pending investment history entries where end_date < today's d
$sql = "SELECT * FROM investment_history WHERE transaction_status = 'approved' AND end_date < CURDATE()";
...
while($row = $result->fetch_assoc()) 
  {
    ...
    $sql = "UPDATE accounts_info SET balance  =  balance + '$new', total_profit = total_profit + '$profit' WHERE user_id = '$user_id'";
    mysqli_query($conn,$sql);
      ...
      while($row_w = $result->fetch_assoc()) 
        {
          ...
          $sql = "UPDATE accounts_info SET referral_bonus  =  referral_bonus  + '$referral_bonus'  WHERE user_id = '$refered_by'";
          mysqli_query($conn,$sql);
