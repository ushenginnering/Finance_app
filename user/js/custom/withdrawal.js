...
let request_withdrawal = (
  ...
  wallet_total
    ...
    loading.start_loading(".withdrawal-btn")
  router.post("http://localhost/finance_app/API'S/USER_API's/withdrawal_request.php", {
    withdraw_type,
    amount_withdrawn:parseInt(amount_withdrawn),
    wallet_address,
    withdraw_from,
    wallet_total:parseInt(wallet_total),
  }, () => loading.stop_loading(".withdrawal-btn", "Complete Withdrawal"))
  .then((data) => {
...
$(function () {
  ...
  $("#withdraw").submit((e) => {
    ...
    let [payment_type, amount, wallet_address] = lowercase(
      $("#payment-type").val(),
      $(".amount").val(),
      $(".wallet-address").val()
    );
